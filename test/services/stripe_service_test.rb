...
class StripeServiceTest < ActiveSupport::TestCase
  ...
  test 'creates stripe checkout session with provided success_url' do
    ...
    StripeService.create_checkout_session(@user, @album, success_url: 'example.com', cancel_url: '')
  end
  ...
  test 'creates stripe checkout session with provided cancel_url' do
    ...
    StripeService.create_checkout_session(@user, @album, success_url: '', cancel_url: 'example.com')
  end
  ...
  test 'creates stripe checkout session using album id as client_reference_id' do
    ...
    StripeService.create_checkout_session(@user, @album, success_url: '', cancel_url: '')
  end
  ...
  test 'returns an ok status if session created successfully' do
    ...
    response = StripeService.create_checkout_session(@user, @album, success_url: '', cancel_url: '')
